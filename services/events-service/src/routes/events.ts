...
export default async function eventRoutes(app: FastifyInstance) {
  ...
  app.post("/events", async (req, reply) => {
    ...
    try {
      const {
        ...
        recurrenceType, recurrenceEndAt, location, visibility
      } = req.body as {
        title: string;
        description?: string;
        startAt: string;
  ...
  app.get("/events", async (req, reply) => {
    ...
    const events = await prisma.event.findMany({
      ...
      // Also select location and visibility explicitly
    });

    // Transform events to include media URLs
    const eventsWithMediaUrls = events.map(event => {
      ...
      return {
        ...event,
        media: event.medias.map(m => `${baseUrl}/uploads/${m.objectKey}`),
        medias: undefined, // Remove raw medias array
        // location and visibility already in event from DB
      };
    });

    return reply.send(eventsWithMediaUrls);
  });
  ...
  app.get("/events/timeline", async (req, reply) => {
    ...
    const events = await prisma.event.findMany({
      ...
      orderBy: { startAt: "asc" },
      include: {
        medias: { select: { id: true, objectKey: true, kind: true } }
      }
    });
    ...
    const eventsWithMedia = events.map(e => ({
      ...e,
      media: e.medias.map(m => `${baseUrl}/uploads/${m.objectKey}`),
      medias: undefined,
    }));
    ...
    const grouped: Record<string, typeof eventsWithMedia> = {};
    for (const e of eventsWithMedia) {
  ...
  app.get("/events/:id", async (req, reply) => {
    ...
    const eventWithMedia = {
      ...ev,
      media: ev.medias.map(m => `${baseUrl}/uploads/${m.objectKey}`),
      medias: undefined,
    };

    const isOwner = userId ? eventWithMedia.ownerId === userId : false;
    const isParticipant = userId ? (eventWithMedia.participants as any[] | undefined)?.length! > 0 : false;
    const hasValidShare = eventWithMedia.visibility === "LINK" && typeof s === "string" && s === eventWithMedia.shareCode;

    if (eventWithMedia.visibility === "PUBLIC") return eventWithMedia;

    if (eventWithMedia.visibility === "LINK") {
      if (isOwner || isParticipant || hasValidShare) return eventWithMedia;
      return reply.code(403).send({ error: "Forbidden" });
    }

    if (eventWithMedia.visibility === "FRIENDS") {
      if (isOwner || isParticipant) return eventWithMedia;
      ...
      const isFriend = await prisma.friendRequest.findFirst({
        where: {
          status: "ACCEPTED",
          OR: [
            { fromUserId: eventWithMedia.ownerId, toUserId: userId },
            { fromUserId: userId, toUserId: eventWithMedia.ownerId },
          ],
        },
        select: { id: true },
      });

      if (isFriend) return eventWithMedia;
      return reply.code(403).send({ error: "Forbidden" });
    }
